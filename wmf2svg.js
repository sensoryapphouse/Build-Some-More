...
class WMFParser {
  ...
  parseMDCR(dataOff, byteCount, recOff) {
    ...
    // For MDCR-V records, the tag is exactly 6 bytes: "MDCR-V".
    // The tag loop may read past byte 5 if byte 6 (low byte of the 2-byte
    // val_size field) happens to be a printable ASCII character (val_size < 256).
    // Reset i to 6 to correctly position past the tag, then read
    // val_size (2 bytes) + padding (2 bytes) to reach the header fields.
    if (baseTag.startsWith('MDCR-V')) {
      i = 6;  // position right after the 6-byte "MDCR-V" tag
      if (i + 4 <= byteCount) {
        subtypeByte = this.u16(dataOff + i);  // 2-byte val_size
        i += 4;  // skip val_size (2) + padding (2) → now at header fields
      }
    }
    ...
    if (baseTag.startsWith('MDCR-V') && subtypeByte !== null) {
...
class EMFParser {
  ...
  parseMDCR(dataOff, byteCount, recOff) {
    ...
    if (baseTag.startsWith('MDCR-V')) { i = 6; if (i + 4 <= byteCount) { subtypeByte = this.u16(dataOff+i); i += 4; } }
    ...
    let fullTag = baseTag;
    if (baseTag.startsWith('MDCR-V') && subtypeByte !== null) { fullTag = 'MDCR-V'; }
...
class SVGRenderer {
  constructor(parsed) {
    ...
    this.curPos = { x: 0, y: 0 };
    this.shapeIndex = 0;
    this.rop2Mode = 13; // R2_COPYPEN (default)
    this.usesBlendMode = false;
  }
  ...
  render() {
    ...
    let svg = `<svg xmlns="${svgNS}" xmlns:mdcr="${mdcrNS}" viewBox="${viewBox}" width="${width}" height="${height}" shape-rendering="geometricPrecision">\n`;
    svg += `  <style>polygon,polyline,line,path{stroke-linejoin:round;stroke-linecap:round}</style>\n`;
    ...
    // When mix-blend-mode is used, add a background rect so the SVG has its
    // own opaque backdrop. CSS filter (e.g. drop-shadow from reward glow)
    // creates an isolation group, which would otherwise leave the blend-mode
    // elements compositing against transparent. The fill defaults to white;
    // the host app should update it to match the activity background colour
    // via the data-blend-bg attribute.
    if (this.usesBlendMode) {
      const vbParts = viewBox.split(/\s+/).map(Number);
      svg += `  <rect data-blend-bg="1" x="${vbParts[0]}" y="${vbParts[1]}" width="${vbParts[2]}" height="${vbParts[3]}" fill="white"/>\n`;
    }
  ...
  renderElements(elements, depth) {
    ...
    for (const el of elements) {
      if (el.type === 'group') {
        ...
        if (el.style) svg += ` style="${el.style}"`;
        if (el.attrs) {
          for (const [k, v] of Object.entries(el.attrs)) {
            svg += ` ${k}="${this.escXml(String(v))}"`;
          }
  ...
  pushElement(el) {
    if (this.rop2Mode !== 13) {
      if (!this.applyROP2(el)) return; // ROP2 says skip this element
    }
    this.getCurrentContainer().push(el);
  }

  applyROP2(el) {
    switch (this.rop2Mode) {
      case 1: // R2_BLACK — force all drawing to black
        if (el.stroke && el.stroke !== 'none') el.stroke = '#000000';
        if (el.fill && el.fill !== 'none') el.fill = '#000000';
        return true;
      case 6: // R2_NOT / INVERT — invert destination pixels
        // Draw white with mix-blend-mode:difference to invert what's underneath
        if (el.stroke && el.stroke !== 'none') el.stroke = '#ffffff';
        if (el.fill && el.fill !== 'none') el.fill = '#ffffff';
        el.style = 'mix-blend-mode:difference';
        this.usesBlendMode = true;
        return true;
      case 7: // R2_XORPEN — XOR pen with destination
        el.style = 'mix-blend-mode:difference';
        this.usesBlendMode = true;
        return true;
      case 11: // R2_NOP — no operation, skip drawing
        return false;
      case 14: // R2_MERGEPENNOT — pen | ~dest, approximate with screen blend
        el.style = 'mix-blend-mode:screen';
        return true;
      case 16: // R2_WHITE — force all drawing to white
        if (el.stroke && el.stroke !== 'none') el.stroke = '#ffffff';
        if (el.fill && el.fill !== 'none') el.fill = '#ffffff';
        return true;
      default:
  ...
  processMDCR(rec) {
    ...
    if (tag.startsWith('MDCR-{')) {
    ...
    } else if (tag.startsWith('MDCR-}')) {
      // Close current group, remember it for subsequent V records
      // Note: some WMFs have trailing chars after } (e.g. MDCR-}')
      if (this.groupStack.length > 0) {
        this.lastClosedGroup = this.groupStack.pop();
      }

    } else if (tag.startsWith('MDCR-S')) {
      ...
      // 2. Buffer for the next drawn shape (per-shape metadata inside a group)
      ...
      if (this.lastClosedGroup) {
        ...
        this.lastClosedGroup = null; // clear so next V record doesn't reuse
...
async function wmfToSvg(url) {
  ...
  if (!resp.ok && resp.status !== 0) throw new Error(`Failed to fetch ${url}: ${resp.status}`);
